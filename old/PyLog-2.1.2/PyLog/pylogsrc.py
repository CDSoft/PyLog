        ...
        if sys.argv[1:] == [ '--build' ]:
                ...
                def GenCode(lines, tag):
                        return {
                                ...
                                #"tpg"   : lambda code: tpg.translate(code).replace('from __future__ import generators',''),
                                "tpg"   : lambda code: code,
                                "prolog": compile,
                        }[tag]("".join(lines))
                
                class TagParser(tpg.Parser):
                    r"""
                        ...
                        TAG/tag -> start/tag ;
                        END<tag> -> stop/s check $s==tag$ ;

                        START/{{"".join(code)}} ->
                                $ code = List()
                                line/l $ code.add(l) $ # #/usr/bin/env python
                                (       TAG/tag
                                                $ block = List()
                                                ( line/l $block.add(l)$ )*
                                        END<tag>
                                        $ code.add(GenCode(block,tag))
                                |       line
                                )*
                                ;
