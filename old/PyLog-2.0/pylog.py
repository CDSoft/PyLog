...
class PrologParser(ToyParser,):
	...
	def _init_scanner(self):
		self._lexer = _Scanner(
			...
			_TokenDef(r"_tok_5", r"\)"),
			_TokenDef(r"_", r"_"),
			_TokenDef(r"_tok_6", r"\["),
			_TokenDef(r"_tok_7", r"\]"),
			_TokenDef(r"py", r"py"),
			_TokenDef(r"_tok_8", r":"),
			_TokenDef(r"_tok_9", r";"),
			_TokenDef(r"_tok_10", r"\|"),
			_TokenDef(r"_tok_11", r"->"),
			_TokenDef(r"_tok_12", r"\*->"),
			_TokenDef(r"_tok_13", r"=\.\."),
			_TokenDef(r"_tok_14", r"=="),
			_TokenDef(r"_tok_15", r"\\=="),
			_TokenDef(r"_tok_16", r"="),
			_TokenDef(r"_tok_17", r"\\="),
			_TokenDef(r"_tok_18", r"<="),
			_TokenDef(r"_tok_19", r"<"),
			_TokenDef(r"_tok_20", r">="),
			_TokenDef(r"_tok_21", r">"),
			_TokenDef(r"_tok_22", r"!"),
			_TokenDef(r"true", r"true"),
			_TokenDef(r"fail", r"fail"),
			_TokenDef(r"_tok_23", r"\\\+"),
			_TokenDef(r"spaces", r"\s+|#.*", None, 1),
			_TokenDef(r"predicate_name", r"[a-z]\w*", None, 0),
			_TokenDef(r"variable_name", r"[A-Z]\w*|_\w+", None, 0),
			_TokenDef(r"string", r"\"(\\.|[^\"\\]+)*\"|'(\\.|[^'\\]+)*'", _cut_string(1), 0),
			_TokenDef(r"floating", r"\d+\.\d*|\.\d+", float, 0),
			_TokenDef(r"integer", r"\d+", int, 0),
		)
...
class _Python:
	def __init__(self, obj): self.obj = obj
	def collectVariables(self, variables): pass
	def genCode(self): return "Atom((%s))"%self.obj.genCode()
	def __str__(self): return str(self.obj)
